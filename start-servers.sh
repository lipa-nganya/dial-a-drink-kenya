#!/bin/bash

# Script to start backend, customer, admin, and ngrok servers

set -e

echo "🚀 Starting Dial a Drink servers..."
echo ""

# Colors for output
GREEN='\033[0;32m'
BLUE='\033[0;34m'
YELLOW='\033[1;33m'
NC='\033[0m' # No Color

# Check if processes are already running
check_port() {
    local port=$1
    if lsof -ti:$port > /dev/null 2>&1; then
        echo -e "${YELLOW}⚠️  Port ${port} is already in use${NC}"
        return 1
    fi
    return 0
}

# Start backend on port 5001
echo -e "${BLUE}📦 Starting Backend Server (port 5001)...${NC}"
if check_port 5001; then
    cd backend
    PORT=5001 npm start > /tmp/backend.log 2>&1 &
    BACKEND_PID=$!
    echo -e "${GREEN}✅ Backend started (PID: $BACKEND_PID)${NC}"
    cd - > /dev/null
    sleep 3
else
    echo -e "${YELLOW}⚠️  Backend port 5001 is in use. Skipping...${NC}"
    BACKEND_PID=""
fi

# Start ngrok for backend
echo -e "${BLUE}🌐 Starting ngrok (port 5001)...${NC}"
if pgrep -f "ngrok" > /dev/null; then
    echo -e "${YELLOW}⚠️  ngrok is already running${NC}"
    NGROK_PID=$(pgrep -f "ngrok")
else
    ngrok http 5001 --log=stdout > /tmp/ngrok.log 2>&1 &
    NGROK_PID=$!
    echo -e "${GREEN}✅ ngrok started (PID: $NGROK_PID)${NC}"
    sleep 3
fi

# Get ngrok URL
echo -e "${BLUE}📋 Fetching ngrok URL...${NC}"
sleep 2
NGROK_URL=$(curl -s http://localhost:4040/api/tunnels 2>/dev/null | grep -o 'https://[^"]*\.ngrok[^"]*' | head -1 || echo "")
if [ -n "$NGROK_URL" ]; then
    echo -e "${GREEN}✅ ngrok URL: ${NGROK_URL}${NC}"
else
    echo -e "${YELLOW}⚠️  Could not fetch ngrok URL. Check http://localhost:4040${NC}"
fi

# Start customer frontend on port 3000
echo -e "${BLUE}🎨 Starting Customer Frontend (port 3000)...${NC}"
if check_port 3000; then
    cd frontend
    npm start > /tmp/frontend.log 2>&1 &
    FRONTEND_PID=$!
    echo -e "${GREEN}✅ Customer Frontend started (PID: $FRONTEND_PID)${NC}"
    cd - > /dev/null
    sleep 3
else
    echo -e "${YELLOW}⚠️  Customer port 3000 is in use. Skipping...${NC}"
    FRONTEND_PID=""
fi

# Start admin frontend on port 3001
echo -e "${BLUE}👨‍💼 Starting Admin Frontend (port 3001)...${NC}"
if check_port 3001; then
    cd admin-frontend
    PORT=3001 npm start > /tmp/admin-frontend.log 2>&1 &
    ADMIN_PID=$!
    echo -e "${GREEN}✅ Admin Frontend started (PID: $ADMIN_PID)${NC}"
    cd - > /dev/null
    sleep 3
else
    echo -e "${YELLOW}⚠️  Admin port 3001 is in use. Skipping...${NC}"
    ADMIN_PID=""
fi

echo ""
echo -e "${GREEN}═══════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}✅ All servers started successfully!${NC}"
echo -e "${GREEN}═══════════════════════════════════════════════════════${NC}"
echo ""
echo -e "${BLUE}📋 Server URLs:${NC}"
echo -e "   Backend:        http://localhost:5001"
echo -e "   Customer Site:  http://localhost:3000"
echo -e "   Admin Panel:    http://localhost:3001"
if [ -n "$NGROK_URL" ]; then
    echo -e "   ngrok:          ${NGROK_URL}"
fi
echo ""
echo -e "${BLUE}📊 Process IDs:${NC}"
[ -n "$BACKEND_PID" ] && echo -e "   Backend:        ${BACKEND_PID}"
[ -n "$FRONTEND_PID" ] && echo -e "   Customer:       ${FRONTEND_PID}"
[ -n "$ADMIN_PID" ] && echo -e "   Admin:          ${ADMIN_PID}"
[ -n "$NGROK_PID" ] && echo -e "   ngrok:          ${NGROK_PID}"
echo ""
echo -e "${YELLOW}💡 Logs are available in /tmp/:${NC}"
echo -e "   Backend:        /tmp/backend.log"
echo -e "   Customer:       /tmp/frontend.log"
echo -e "   Admin:          /tmp/admin-frontend.log"
echo -e "   ngrok:          /tmp/ngrok.log"
echo ""
echo -e "${YELLOW}💡 To stop all servers, run: ./stop-servers.sh${NC}"
echo ""
