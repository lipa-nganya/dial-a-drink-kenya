...
class AllPushNotificationsFragment : Fragment() {
    private var _binding: FragmentNotificationsListBinding? = null
    private val binding get() = _binding!!
    
    private var notifications = mutableListOf<PushNotification>()
    private val dateTimeFormat = SimpleDateFormat("MMM dd, yyyy HH:mm", Locale.getDefault()).apply {
    ...
    private fun displayNotifications() {
        ...
        // Remove only notification cards and day headers, not the empty state text
        ...
        if (notifications.isEmpty()) {
            ...
            binding.emptyStateText.text = "No push notifications saved"
            return
        }
        ...
        sortedDays.forEach { dayString ->
            ...
            dayNotifications.forEach { notification ->
                ...
                previewText.text = notification.message
                
                // Format date and time
                try {
                    val date = parseDate(notification.sentAt)
                    // Show only time (HH:mm format)
                ...
                card.setOnClickListener {
                    val details = buildString {
                        ...
                        append("Sent: ${dateTimeFormat.format(parseDate(notification.sentAt))}")
                        if (notification.isRead) {
                            append("\nRead: ${notification.readAt ?: "Yes"}")
                        }
                    }
                    android.widget.Toast.makeText(requireContext(), details, android.widget.Toast.LENGTH_LONG).show()
                }
