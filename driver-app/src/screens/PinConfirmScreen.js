...
const PinConfirmScreen = ({ route, navigation }) => {
  ...
  const handleConfirm = async () => {
    ...
    setLoading(true);
    try {
      // Save PIN locally (use the confirmed PIN value)
      await AsyncStorage.setItem('driver_pin', String(confirmPin));
      await AsyncStorage.setItem('driver_phone', String(phoneNumber));
      
      // Update driver's last activity
      try {
        await api.patch(`/drivers/phone/${phoneNumber}/activity`);
      } catch (error) {
      ...
      console.log('PIN confirmed and saved successfully');
