#!/bin/bash
# Script to set up DATABASE_URL for Cloud Run backend
...
echo "🔧 Setting up DATABASE_URL for Cloud Run backend..."
echo ""
...
if [ -z "$1" ]; then
    ...
    echo ""
    echo "Then run this script with the new password:"
    echo "  $0 NEW_PASSWORD"
    exit 1
fi
...
echo "🚀 Updating Cloud Run service with DATABASE_URL..."
...
echo ""
echo "📝 Next steps:"
echo "   1. Wait for the new revision to deploy (about 1-2 minutes)"
echo "   2. Test the backend: curl https://dialadrink-backend-910510650031.us-central1.run.app/api/health"
echo "   3. Try admin login again"
echo ""
