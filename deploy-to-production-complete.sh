...
if [ -n "$EXISTING_ENV_YAML" ]; then
    ...
    PRESERVED_VAR_NAMES=$(echo "$EXISTING_ENV_YAML" | grep "name:" | sed "s/.*name: //" | grep -vE "FRONTEND_URL|ADMIN_URL|GOOGLE_CLOUD_PROJECT|GCP_PROJECT|HOST|NODE_ENV|DATABASE_URL|MPESA_CONSUMER_KEY|MPESA_CONSUMER_SECRET|MPESA_SHORTCODE|MPESA_PASSKEY|MPESA_ENVIRONMENT|MPESA_CALLBACK_URL|PESAPAL_CONSUMER_KEY|PESAPAL_CONSUMER_SECRET|PESAPAL_ENVIRONMENT|PESAPAL_IPN_CALLBACK_URL|GOOGLE_MAPS_API_KEY" || echo "")
...
gcloud run deploy "$BACKEND_SERVICE" \
    ...
    --set-env-vars "NODE_ENV=production,DATABASE_URL=$PROD_DATABASE_URL,FRONTEND_URL=$EXISTING_FRONTEND_URL,ADMIN_URL=$EXISTING_ADMIN_URL,GOOGLE_CLOUD_PROJECT=$EXISTING_GOOGLE_CLOUD_PROJECT,GCP_PROJECT=$EXISTING_GCP_PROJECT,HOST=$EXISTING_HOST,MPESA_CONSUMER_KEY=$MPESA_CONSUMER_KEY,MPESA_CONSUMER_SECRET=$MPESA_CONSUMER_SECRET,MPESA_SHORTCODE=$MPESA_SHORTCODE,MPESA_PASSKEY=$MPESA_PASSKEY,MPESA_ENVIRONMENT=$MPESA_ENVIRONMENT,MPESA_CALLBACK_URL=$MPESA_CALLBACK_URL,PESAPAL_CONSUMER_KEY=$PESAPAL_CONSUMER_KEY,PESAPAL_CONSUMER_SECRET=$PESAPAL_CONSUMER_SECRET,PESAPAL_ENVIRONMENT=$PESAPAL_ENVIRONMENT,PESAPAL_IPN_CALLBACK_URL=$PESAPAL_IPN_CALLBACK_URL,GOOGLE_MAPS_API_KEY=$GOOGLE_MAPS_API_KEY" \
    --memory 512Mi \
    --timeout 300 \
    --max-instances 10 \
    --min-instances 0 \
