...
import { showPushNotificationOverlay } from './src/services/overlayService';
...
const App = () => {
  ...
  // Export checkForUpdates so it can be called from other screens
  ...
  const checkForUpdates = async () => {
    try {
      ...
      if (update.isAvailable) {
        ...
        // Increment OTA update count before reloading
        try {
          const currentCount = await AsyncStorage.getItem('ota_update_count');
          const newCount = currentCount ? parseInt(currentCount, 10) + 1 : 1;
          await AsyncStorage.setItem('ota_update_count', newCount.toString());
          console.log(`📱 OTA update count incremented to: ${newCount}`);
        } catch (error) {
          console.error('❌ Error saving OTA count:', error);
        }
  ...
  // Configure notification channels and request permissions immediately on app start
  // This ensures push notifications work properly in standalone builds
  useEffect(() => {
    const setupNotifications = async () => {
      try {
        ...
        // Request notification permissions early (required for Android 13+ and iOS)
        ...
        if (finalStatus === 'granted') {
          ...
          if (driverPhone && isLoggedIn === 'true') {
            try {
              console.log('📱 Driver is logged in, fetching driver info...');
              const driverResponse = await api.get(`/drivers/phone/${driverPhone}`);
              
              if (driverResponse.data?.id) {
                const driverId = driverResponse.data.id;
                console.log('📱 ===== REGISTERING FOR PUSH NOTIFICATIONS ON APP START =====');
                ...
                if (token) {
                  console.log('✅ ✅ ✅ Push notifications registered successfully on app start');
                  console.log('✅ Token obtained (first 50 chars):', token.substring(0, 50) + '...');
                } else {
                  console.error('❌ ❌ ❌ Push notification registration returned NULL token on app start');
              ...
              console.error('❌ ❌ ❌ CRITICAL ERROR registering for push notifications on app start:');
            ...
            console.log('ℹ️ Driver not logged in yet - push notifications will be registered after login');
          }
        } else {
          console.log('❌ Notification permissions denied or not granted');
          console.log('⚠️ Push notifications will not work until permissions are granted');
        }
      } catch (error) {
        console.error('❌ Error setting up notifications on app start:', error);
        console.error('❌ Error stack:', error.stack);
      }
    };
    ...
    // Also set up listener for app state changes to re-register when app comes to foreground
    const subscription = AppState.addEventListener('change', async (nextAppState) => {
      if (nextAppState === 'active') {
        console.log('📱 App came to foreground - checking push notification registration...');
        try {
          const driverPhone = await AsyncStorage.getItem('driver_phone');
          const isLoggedIn = await AsyncStorage.getItem('driver_logged_in');
          ...
          if (driverPhone && isLoggedIn === 'true') {
            ...
            if (status === 'granted') {
              try {
                const driverResponse = await api.get(`/drivers/phone/${driverPhone}`);
                if (driverResponse.data?.id) {
                  console.log('📱 Re-registering for push notifications on app foreground...');
                  await registerForPushNotifications(driverResponse.data.id);
                  console.log('✅ Push notifications re-registered on app foreground');
                }
              } catch (error) {
                console.error('❌ Error re-registering push notifications on foreground:', error);
              }
            }
          }
        } catch (error) {
          console.error('❌ Error checking push notifications on app state change:', error);
        }
      }
    });
  ...
  useEffect(() => {
    ...
    const handleNotificationEvent = async (notification) => {
      ...
      try {
        ...
        if (driverPhone) {
          const driverResponse = await api.get(`/drivers/phone/${driverPhone}`);
          if (driverResponse.data?.name) {
            currentDriverName = driverResponse.data.name;
            setDriverName(currentDriverName);
          }
        }
      } catch (error) {
        console.error('❌ Error loading driver name for overlay:', error);
      }
      
      console.log('📱 Showing root-level overlay from event, driverName:', currentDriverName);
      // Show overlay at root level - works in production builds
      setTimeout(() => {
        showPushNotificationOverlay(currentDriverName);
      }, 100);
    };
    ...
    const receivedSubscription = Notifications.addNotificationReceivedListener(async (notification) => {
      ...
      console.log('📱 Showing root-level overlay, driverName:', currentDriverName);
      // Show overlay at root level - works in production builds
      setTimeout(() => {
        showPushNotificationOverlay(currentDriverName);
      }, 100);
    ...
    const responseSubscription = Notifications.addNotificationResponseReceivedListener(async (response) => {
      ...
      console.log('📱 Showing root-level overlay from response, driverName:', currentDriverName);
      // Show overlay at root level - works in production builds
      setTimeout(() => {
        showPushNotificationOverlay(currentDriverName);
      }, 200);
    ...
    // Handle app state changes - check for OTA updates and notifications when app comes to foreground
    const handleAppStateChange = async (nextAppState) => {
      console.log('📱 App state changed to:', nextAppState);
      if (nextAppState === 'active') {
        console.log('📱 App came to foreground, checking for updates and notifications...');
        
        // Check for OTA updates when app comes to foreground (ensures drivers always have latest)
        if (!__DEV__ && Updates && Updates.isEnabled) {
          try {
            console.log('🔄 Checking for OTA updates on foreground...');
            const update = await Updates.checkForUpdateAsync();
            if (update.isAvailable) {
              console.log('✅ Update available on foreground! Downloading...');
              const fetchResult = await Updates.fetchUpdateAsync();
              if (fetchResult.isNew) {
                console.log('✅ New update downloaded, incrementing OTA count...');
                try {
                  const currentCount = await AsyncStorage.getItem('ota_update_count');
                  const newCount = currentCount ? parseInt(currentCount, 10) + 1 : 1;
                  await AsyncStorage.setItem('ota_update_count', newCount.toString());
            ...
            console.error('❌ Error checking for updates on foreground:', updateError);
          }
        }
        ...
        try {
          ...
          if (lastNotificationResponse) {
            ...
            console.log('📱 Showing root-level overlay from last notification, driverName:', currentDriverName);
            // Show overlay at root level - works in production builds
            setTimeout(() => {
              showPushNotificationOverlay(currentDriverName);
            }, 300);
  ...
  console.log('🎨 App render - driverName:', driverName);
  ...
  return (
    <ThemeProvider>
      ...
      {/* Overlay is now rendered at root level using react-native-root-siblings */}
      {/* This ensures it works in production builds by bypassing React Native's view hierarchy */}
    </ThemeProvider>
  );
};
