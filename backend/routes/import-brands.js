...
// Import brands from old site data
router.post('/import-brands', async (req, res) => {
  try {
    ...
    res.json({
      message: 'Brands import completed',
      added: addedBrands.length,
      existing: existingBrands.length,
      errors: errors.length,
      details: {
        added: addedBrands.map(b => ({ id: b.id, name: b.name })),
        existing: existingBrands.map(b => ({ id: b.id, name: b.name })),
        errors: errors
      }
    });
  } catch (error) {
    console.error('Error importing brands:', error);
    res.status(500).json({ error: 'Failed to import brands', details: error.message });
  }
});
