...
async function checkOrder431() {
  try {
    // Find order 431
    const order = await db.Order.findByPk(431, {
      attributes: [
    ...
    if (!order) {
      console.log('Order 431 not found');
      process.exit(1);
    }
    ...
    // Get all transactions for order 431
    ...
    console.log('\n=== ALL TRANSACTIONS FOR ORDER 431 ===');
