...
async function checkOrder428() {
  try {
    // Find order 428
    const order = await db.Order.findByPk(428, {
      attributes: [
    ...
    if (!order) {
      console.log('Order 428 not found');
      process.exit(1);
    }
    ...
    // Get all transactions for order 428
    ...
    console.log('\n=== ALL TRANSACTIONS FOR ORDER 428 ===');
